...
export default function MarsScene({
  ...
  active: boolean;
  onTelemetry: (value: RoverTelemetry) => void;
  ...
  return (
    ...
    >
      <color attach="background" args={[SPEC.palette.void]} />
      <fog
        attach="fog"
        args={[SPEC.fog.color, SPEC.fog.near, SPEC.fog.far]}
      />
      ...
      {destinations.map((site) => (
        <DestinationMarker
          ...
          active={
            telemetry.nearestId === site.id &&
            telemetry.distance < 7
          }
          onAccess={onAccess}
        />
      ))}
      ...
      <RoverCamera roverRef={roverRef} calm={false} />
    </Canvas>
  );
}
